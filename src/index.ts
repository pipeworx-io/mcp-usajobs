...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'USAJOBS.gov');
}
...
const tools: McpToolExport['tools'] = [
  ...
  {
    name: 'list_agencies',
    description: 'Return the USAJOBS reference list of all federal agency sub-element codes and names; useful for resolving agency codes returned by the search tool.',
    inputSchema: { type: 'object', properties: {} },
  },
  {
    name: 'list_pay_grades',
    description: 'Return the USAJOBS reference list of pay plan codes (GS, FW, ES, etc.) and their descriptions; use pay plan codes with the pay_grade_low/pay_grade_high search filters.',
    inputSchema: { type: 'object', properties: {} },
  },
  {
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  // The /codelist/* endpoints are public reference data and accept the
  // default User-Agent without an Authorization-Key. Only /search and
  // /get_job require a registered key + email-format UA.
  const codesTools = new Set(['list_agencies', 'list_pay_plans', 'list_pay_grades', 'list_occupational_series']);
  const apiKey = (args._apiKey as string | undefined)?.trim();
  if (!apiKey && !codesTools.has(name)) {
    throw new Error(
  ...
  const colon = apiKey ? apiKey.indexOf(':') : -1;
  const authKey = !apiKey ? '' : (colon === -1 ? apiKey : apiKey.slice(0, colon));
  const userAgent = !apiKey || colon === -1 ? DEFAULT_UA : apiKey.slice(colon + 1);
  ...
  switch (name) {
    ...
    case 'list_agencies':
      // Was `/agencysubelement` (singular, 404). Correct path is plural.
      return codesGet(authKey, userAgent, '/agencysubelements');
    case 'list_pay_grades':
    case 'list_pay_plans':
      // USAJOBS exposes /payplans (GS, FW, etc.), not /paygrades. Accept
      // both tool names so the old list_pay_grades binding keeps working.
      return codesGet(authKey, userAgent, '/payplans');
